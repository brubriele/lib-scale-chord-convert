...
const libConvert = (fNote, transpositionKey, scale) => {    
  const indexNote = notes.indexOf(fNote.toUpperCase());

  const chosenNote = scaleGroup[scale.toLowerCase()].map((note) => {
    return transpositionKey > 0 ? (note + indexNote + transpositionKey) % 12 : 
      (note + indexNote - transpositionKey) % 12;
  });
